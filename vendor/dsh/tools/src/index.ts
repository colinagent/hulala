 ...
 * execution pipeline.
 * @module @hulala/dsh-tools
 */

import { Context, Service } from '@hulala/cordis'
import z from '@hulala/schemastery'
import { AnonymousEntries, NamedEntries, ScopedLayers, scopeOf, scopeTarget } from '@hulala/dsh-scope'
import type { ScopeKey, ScopeLayer, Scoped } from '@hulala/dsh-scope'
import type { CallId, ContentBlock, ToolSchema } from '@hulala/dsh-llm'
import { assertNever, deepFreeze, HarnessError } from '@hulala/dsh-llm'
import type { Agent } from '@hulala/dsh-agent'
import { snapshotJsonValue } from '@hulala/dsh-session'
import type { JsonValue, UserMessage } from '@hulala/dsh-session'
import type { ToolProviderResult } from '@hulala/dsh-system-prompt'
import type { CodeRuntime } from '@hulala/dsh-code-runtime'
// Type-only: makes `ctx.get('approval')` resolve to the ApprovalService
// augmentation. The seam stays optional at runtime — see `serviceAsk`.
import type {} from '@hulala/dsh-user-approval'
...
export type { JsonValue } from '@hulala/dsh-session'
...
// lives in its own UI-facing module; re-export it so `@hulala/dsh-tools`
...
declare module '@hulala/cordis' {
  ...
  interface Events {
     ...
     * Scope-filtered dispatch (`@hulala/dsh-scope`): agent-scoped listeners receive only that agent's calls.
     ...
     * Scope-filtered dispatch (`@hulala/dsh-scope`): agent-scoped listeners receive only that agent's dispatches.
     ...
     * Scope-filtered dispatch (`@hulala/dsh-scope`): keyed by `exec.agent`.
...
export interface ToolDefinition extends ToolSchema {
   ...
   * Enforced by `@hulala/dsh-tool-call-timeout-policy` (a `tools/execute` wrapper); it
 ...
 * @internal
 */
export const TOOL_RUNTIME_SCHEDULER: unique symbol = Symbol('@hulala/dsh-tools.scheduler')
...
export class ToolRuntime extends Service {
  ...
  private requireCodeRuntime(mode: ToolPresentationMode): CodeRuntime {
    ...
    if (!runtime) {
      throw new Error(`dsh-tools: mode "${mode}" requires a code runtime — load a ctx.codeRuntime implementation (e.g. @hulala/dsh-code-runtime-worker-thread) or set tools mode to "native"`)
    }
