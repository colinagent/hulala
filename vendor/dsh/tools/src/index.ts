 ...
 * execution pipeline.
 * @module @loopwithai/dsh-tools
 */

import { Context, Service } from '@loopwithai/cordis'
import z from '@loopwithai/schemastery'
import { AnonymousEntries, NamedEntries, ScopedLayers, scopeOf, scopeTarget } from '@loopwithai/dsh-scope'
import type { ScopeKey, ScopeLayer, Scoped } from '@loopwithai/dsh-scope'
import type { CallId, ContentBlock, ToolSchema } from '@loopwithai/dsh-llm'
import { assertNever, deepFreeze, HarnessError } from '@loopwithai/dsh-llm'
import type { Agent } from '@loopwithai/dsh-agent'
import { snapshotJsonValue } from '@loopwithai/dsh-session'
import type { JsonValue, UserMessage } from '@loopwithai/dsh-session'
import type { ToolProviderResult } from '@loopwithai/dsh-system-prompt'
import type { CodeRuntime } from '@loopwithai/dsh-code-runtime'
// Type-only: makes `ctx.get('approval')` resolve to the ApprovalService
// augmentation. The seam stays optional at runtime — see `serviceAsk`.
import type {} from '@loopwithai/dsh-user-approval'
...
export type { JsonValue } from '@loopwithai/dsh-session'
...
// lives in its own UI-facing module; re-export it so `@loopwithai/dsh-tools`
...
declare module '@loopwithai/cordis' {
  ...
  interface Events {
     ...
     * Scope-filtered dispatch (`@loopwithai/dsh-scope`): agent-scoped listeners receive only that agent's calls.
     ...
     * Scope-filtered dispatch (`@loopwithai/dsh-scope`): agent-scoped listeners receive only that agent's dispatches.
     ...
     * Scope-filtered dispatch (`@loopwithai/dsh-scope`): keyed by `exec.agent`.
...
export interface ToolDefinition extends ToolSchema {
   ...
   * Enforced by `@loopwithai/dsh-tool-call-timeout-policy` (a `tools/execute` wrapper); it
 ...
 * @internal
 */
export const TOOL_RUNTIME_SCHEDULER: unique symbol = Symbol('@loopwithai/dsh-tools.scheduler')
...
export class ToolRuntime extends Service {
  ...
  private requireCodeRuntime(mode: ToolPresentationMode): CodeRuntime {
    ...
    if (!runtime) {
      throw new Error(`dsh-tools: mode "${mode}" requires a code runtime — load a ctx.codeRuntime implementation (e.g. @loopwithai/dsh-code-runtime-worker-thread) or set tools mode to "native"`)
    }
